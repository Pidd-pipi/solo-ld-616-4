...
- 测试：在 `backend` 下执行 `npm test`（`pretest` 会先 `tsc` 构建）。

### 集成测试（真实存储，无 mock）

`backend/test/lifecycle.persistence.test.ts` 使用**真实 PostgreSQL**（`embedded-postgres` 拉起的真实 postgres 进程 + 持久数据目录）与**真实后端子进程**（构建产物 `dist/main.js`），不使用内存替身、mock 或单连接串行化来绕开持久化、行锁与接管；断言通过独立连接池从数据库**可回读**验证。每轮使用全新数据目录与唯一键/编码，可连续重复运行，结果稳定。

| 覆盖场景 | 阶段与可回读后果 |
|---|---|
| 基线种子与台账视图 | 设备 3、计划/证书/预警各 3；每行含生命周期/校准状态/最近变更 |
| 同接口同键·串行重放 | 两次同结果；`device_lifecycle_record` 中 EXEMPT 仅 1 条；幂等行 status_code=200 |
| 同接口同键·并发重放 | 8 个真实连接 `Promise.all`：全部成功且回读同一 id；该编码设备仅 1 台、幂等行仅 1 条 |
| 失联占位接管 | `status_code=0` 且租约过期的死占位被安全接管；SCRAP 仅 1 条；同键再请求重放 200；占位 owner 已切换并 completed |
| 存活持有者不被接管 | 独立连接 `BEGIN … FOR UPDATE` 真持锁期间，跟随者等待后得可重试 `409`，零业务写入；锁释放且租约过期后接管成功，仍仅 1 条 |
| 业务失败回收 | 报废冲突 409 后未完成占位被删除、设备保持 ACTIVE；同键改目标重试成功且 SCRAP 仅 1 条 |
| 跨接口同键隔离 | 建档/豁免/报废复用一键各返回本接口结果；3 个 scope 各一行；三类副作用各 1 次 |
| 断库恢复 | 停库后进程存活、`/health=503 database=down`、读写 503 且 `retryable:true` + `Retry-After`；恢复后自动就绪、数量不变、断库期间写入不入库、恢复后写入 +1 |
| 进程重启持久化 | 重启后端（库不动）后豁免原因/状态/最近变更与新建设备可回读；计划/证书/预警数量不变 |

失败信息统一带 `[阶段名]` 前缀与 `DB回读=...` 快照，可直接定位发生阶段和数据库后果。运行时数据写入 `backend/.tmp/`（已在 `.gitignore`）。相关时间参数可用 `IDEMPOTENCY_LEASE_TTL_MS` 等环境变量调整。
